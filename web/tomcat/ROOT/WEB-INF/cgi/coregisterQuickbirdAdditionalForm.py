...
#-*- coding: utf-8 -*-
"""
This program generates an OGS WPS query for quickbird additional band image coregistration.
"""
...
def main():
    ...
    print "<html><head><title>Quickbird Additional Band Coregistration</title>"
    print "</head><body>"
    ...
    fnamein = form[ 'fnamein' ].value.translate(sc.pathSafeTrans)
    # band isn't a path, but this translation will filter out anything dangerous
    band = form[ 'band' ].value.translate(sc.pathSafeTrans)
    gridtar = form[ 'gridtar' ].value.translate(sc.pathSafeTrans)
    ...
    url = sc.schemeHostPort + "/cgi-bin/wps.py?service=Wps&version=0.4.0&request=execute&identifier=coregisterquickbirdadditional&datainputs=prefix," + prefix  + \
          ",rawimg," + uploadDirUrl + "/" + rawimgname + \
          ",rawmeta," + uploadDirUrl + "/" + rawmetaname + \
          ",fnamein," + outputDirUrl + "/" + fnamein + \
          ",band," + band + \
          ",gridtar," + outputDirUrl + "/" + gridtar + \
          ",userdir," + userDir + \
