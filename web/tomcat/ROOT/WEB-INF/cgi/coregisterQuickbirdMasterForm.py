...
#-*- coding: utf-8 -*-
"""
This program generates an OGS WPS query for quickbird master image coregistration.
"""
...
def main():
    ...
    print "<html><head><title>Quickbird Master Coregistration</title>"
    print "</head><body>"
    ...
    url = sc.schemeHostPort + "/cgi-bin/wps.py?service=Wps&version=0.4.0&request=execute&identifier=coregisterquickbirdmaster&datainputs=prefix," + prefix + \
    ...
    outnitfUrl = sc.getURL(data1, "outnitf")
    logUrl = sc.getURL(data1, "log")
    outgridUrl = sc.getURL(data1, "outgrid")
    outvicarUrl = sc.getURL(data1, "outvicar")
    geoaccUrl = sc.getURL(data1, "geoacc")
    outplotUrl = sc.getURL(data1, "outplot")
    ...
    statinfo = os.stat( wpsoutput + "/" + os.path.basename(outnitfUrl) )
    if ( statinfo.st_size == 0 ):
        print "<font color=\"red\">Processing Failure</font>"
        badFile = 1
    else:
        print "<a href=\"" + outnitfUrl + "\">" + os.path.basename(outnitfUrl) + "</a>"
    ...
    statinfo = os.stat( wpsoutput + "/" + os.path.basename(geoaccUrl) )
    if ( statinfo.st_size == 0 ):
        print "<font color=\"red\">Processing Failure</font>"
        badFile = 1
    else:
        print "<a href=\"" + geoaccUrl + "\">" + os.path.basename(geoaccUrl) + "</a>"

    print "</td><td>Accuracy statistics"
    print "</td></tr>"
    print "<tr><td>"

    statinfo = os.stat( wpsoutput + "/" + os.path.basename(outplotUrl) )
    if ( statinfo.st_size == 0 ):
        print "<font color=\"red\">Processing Failure</font>"
        badFile = 1
    else:
        print "<a href=\"" + outplotUrl + "\">" + os.path.basename(outplotUrl) + "</a>"

    print "</td><td>Accuracy plot"
    print "</td></tr>"
    print "<tr><td>"
