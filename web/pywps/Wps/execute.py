...
class Execute:
    ...
    def __init__(self, serverSettings, grassSettings, process, formvalues,):
        ...
        self.timestamp = "%02d%02d%02d%02d%02d%02d" % (time.localtime()[0:6])
        ...
        userdir = self.formvalues['datainputs']['userdir']
        ...
        try:
            ...
            if self.process.statusSupported == "true" and \
               ...
               self.formvalues['store'].lower() == "true":
                ...
                try:
                # It was originally: changed by Peter Kim
                #    self.statusLocation = self.settings.ServerSettings['outputUrl']+"/"+self.executeresponseXmlName
                    self.statusLocation = sc.usersUrl + "/" + userdir + "/wpsoutput/"+self.executeresponseXmlName
                    self.pid = os.fork() 
                    if self.pid:
                        self.make_response_xml()
                        file = open(
                                # rerouted by Peter Kim
                                # os.path.join(self.settings.ServerSettings['outputPath'],self.executeresponseXmlName),"w")
                                sc.usersDir + "/" + userdir + "/wpsoutput/"+self.executeresponseXmlName, "w")
                        file.write(self.document.toprettyxml())
                        file.close()
                        return
                    else:
                        ...
                        status = Status(document=self.document,
                                    # Redirected by Peter Kim
                                    # filename=os.path.join(
                                    #    self.settings.ServerSettings['outputPath'],
                                    #    self.executeresponseXmlName),
                                    filename=sc.usersDir + "/" + userdir + "/wpsoutput/" + self.executeresponseXmlName,
                                    interval=1,
    ...
    def execute(self):
        ...
        #os.system( "echo line_261 >> /tmp/wpstrace" )
        #
        # make GRASS MAPSET.
        ...
        #os.system( "echo line_271 >> /tmp/wpstrace" )
        try:
        ...
        #os.system( "echo line_279 >> /tmp/wpstrace" )
        #
        ...
        #os.system( "echo line_305 >> /tmp/wpstrace" )
        #######
        # MAKE
        ...
        try:
            #os.system( "echo executingProcess >> /tmp/wpstrace" )
            error = self.process.execute()
            #os.system( "echo executedProcess >> /tmp/wpstrace" )
    ...
    def mktempdir(self,directory=None):
        ...
        try:
            ...
            print "Content-type: text/xml\n"
            print self.wpsExceptions.make_exception(
                    "ServerError4","IOError, OSError: tempdir not created - %s" % what)
            self.document = None
            return
        #########
