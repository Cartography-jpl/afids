...
static int compare_ind(const void* u_vp, const void* v_vp) {
  const int* u = (const int*) u_vp;
  const int* v = (const int*) v_vp;

  return pt[*u].y <= pt[*v].y ? -1 : 1;
}
static int compare_active(const void* u_vp, const void* v_vp) {
  const Edge* u = (const Edge*) u_vp;
  const Edge* v = (const Edge*) v_vp;

  return u->x <= v->x ? -1 : 1;
}

static void cdelete(int);
static void cinsert(int, int);
