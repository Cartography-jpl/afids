...
#include "defines.h"
#include "carto/ibisControlMapper.h"
...
void cluster(IBISStruct *ibis, IBIS_CONTROL_MAP *map)
{
   ...
   distMatrix = (double**)malloc(sizeof(double*)*(long unsigned int)map->length);
   linkMatrix = (int**)malloc(sizeof(int*)*(long unsigned int)map->length);
   for(i = 0; i < map->length; i++)
   {
      distMatrix[i] = (double*)calloc((long unsigned int)map->length, sizeof(double));
      linkMatrix[i] = (int*)calloc((long unsigned int)map->length, sizeof(int));
   }
   neighborCnt = (int*)calloc((long unsigned int)map->length, sizeof(int));
   inCluster = (int*)calloc((long unsigned int)map->length, sizeof(int));
...
void main44(void)
{
   ...
   IBISStruct *ibis;
    ...
    zvmessage("ibisclst - 10-09-2019 - rjb - 64-bit"," ");
