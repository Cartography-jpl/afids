...
void main44(void)
{
   ...
   int labnl,labns,in_rot,wrot=0,wcode=0,outnl,outns;
   int scaletype,ier;
   ...
   zifmessage("mvecall version Fri Jan 11 2008");
   ...
   printf("\nChanging rotation %d to rotation %d",in_rot,out_rot);
   if (out_rot==1) printf(" (the standard VICAR rotation)");
   printf("\n\n");
   ...
   else zmabend("This image type not handled by mvecall");
