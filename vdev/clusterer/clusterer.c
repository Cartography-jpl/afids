...
void printSample(SAMPLE* s)
{
   ...
   sprintf(message,"line: %d samp: %d\n", s->line, s->samp);
    zvmessage(message," ");
   for(i = 0; i < (int)s->dns->size; i++) {
        sprintf(message,"%lf ", gsl_vector_get(s->dns, (size_t)i));
        zvmessage(message," ");
    }
   zvmessage("\n"," ");
}
...
void printCluster(CLUSTER *c)
{
   ...
   sprintf(message,"-----> CLUSTER: %d nSamples: %d\n", c->c->id, c->nSamples);
    zvmessage(message," ");
   for(i = 0; i < (int)c->c->means->size; i++) {
        sprintf(message,"%lf ", gsl_vector_get(c->c->means, (size_t)i));
        zvmessage(message," ");
        zvmessage("\n"," ");
    }
   for(i = 0; i < (int)c->oldMeans->size; i++) {
        sprintf(message,"%lf ", gsl_vector_get(c->oldMeans, (size_t)i));
        zvmessage(message," ");
        zvmessage("\n"," ");
    }
...
TMP* getTMP(int nClusters, int nDim)
{
   ...
   tmp->diff = gsl_vector_alloc((size_t)nDim);
   tmp->deltas = (double*)malloc(sizeof(double)*(long unsigned int)nClusters);
...
SAMPLE** readSamples(int *nSamples, int *nDim)
{
   ...
   *nDim = inp->nc-2;
   *nSamples = inp->nr;
   samples = (SAMPLE**)malloc(sizeof(SAMPLE*)*((long unsigned int)*nSamples));
   ...
   for(i = 0; i < *nSamples; i++)
   {
      ...
      sample->dns = gsl_vector_alloc((size_t)*nDim);
      for(j = 0; j < *nDim; j++) {
         gsl_vector_set(sample->dns, (size_t)j, IBISHELPER_getDouble(inp, j+2, i));
        }
      samples[i] = sample;
   }
...
void setBoundingBox(SAMPLE** samples, double* mins, double* maxes, int nSamples, int nDim)
{
   ...
   for(i = 0; i < nSamples; i++)
   {
      for(j = 0; j < nDim; j++)
      {
         double dn = gsl_vector_get(samples[i]->dns, (size_t)j);
...
void initializeCluster(CLUSTER* cluster, int id, double* mins, double* maxes, int nDim)
{
   ...
   means = gsl_vector_alloc((size_t)nDim);
   for(j = 0; j < (int)means->size; j++)
      gsl_vector_set(means, (size_t)j, rand()/(double)RAND_MAX*(mins[j]+maxes[j]));
...
void binSamplesIntoClusters(CLUSTER** clusters, SAMPLE** samples, int nClusters, int nSamples)
{
   ...
   tmp = CARTOCLASS_getClassTmp((int)clusters[0]->c->means->size);
   //   printf("here1\n");
...
void calculateCentroid(CLUSTER* cluster)
{
   ...
   for(i = 0; i < (int)cluster->c->means->size; i++) gsl_vector_set(cluster->c->means, (size_t)i, 0.);
   ...
   for(i = 0; i < (int)cluster->c->means->size; i++)
      gsl_vector_set(cluster->c->means, (size_t)i, gsl_vector_get(cluster->c->means, (size_t)i)/(double)cluster->nSamples);
}
...
double l2Norm1(gsl_vector *diff)
{
   ...
   sum = 0;
   for(i = 0; i < (int)diff->size; i++) sum += pow(gsl_vector_get(diff, (size_t)i), 2.);
...
//   int changed;
   ...
   sprintf(message,"iter: %d total changed: %lf theshold: %lf\n", iteration, totalChanged, convergenceThreshold);
    zvmessage(message," ");
   if(fabs(totalChanged - convergenceThreshold) < 10E-10) return 1;
   return 0;
}
...
void printIteration(CLUSTER** clusters, TMP *tmp, int nClusters, int nIteration)
{
   ...
   for(i = 0; i < nClusters; i++)
   {
      ...
      sprintf(message,"diff: %lf\n", tmp->deltas[i]);
      sum += tmp->deltas[i];
   }
   sprintf(message,"=========================\n");
    zvmessage(message," ");
   sprintf(message,"total shift: %lf %d\n", sum, nIteration);
    zvmessage(message," ");
}
...
CLUSTER** getClusters(SAMPLE** samples, int nClusters, int nSamples, int nDim, TMP *tmp, double thresh, int maxIterations, int *convergedFlag)
{
   ...
   clusters = (CLUSTER**)malloc(sizeof(CLUSTER*)*(long unsigned int)nClusters);
   for(i = 0; i < nClusters; i++)
   {
      clusters[i] = (CLUSTER*)malloc(sizeof(CLUSTER));
      clusters[i]->samples = (SAMPLE**)malloc(sizeof(SAMPLE*)*(long unsigned int)nSamples);
      clusters[i]->oldMeans = NULL;
      clusters[i]->alive = 1;
   }
   mins = (double*)malloc(sizeof(double)*(long unsigned int)nDim);
   maxes = (double*)malloc(sizeof(double)*(long unsigned int)nDim);

   sprintf(message,"setting bounding box\n");
    zvmessage(message," ");
   setBoundingBox(samples, mins, maxes, nSamples, nDim);

   sprintf(message,"initializing clusters\n");
    zvmessage(message," ");
   for(i = 0; i < nClusters; i++) {
        initializeCluster(clusters[i], i+1, mins, maxes, nDim);
    }
   sprintf(message,"binning clusters\n");
    zvmessage(message," ");
...
int calculateCov(CLUSTER* cluster, int nDim)
{
   ...
   if(cluster->c->cov_matrix == NULL)
      cluster->c->cov_matrix = gsl_matrix_alloc((size_t)nDim, (size_t)nDim);

   if(cluster->nSamples == 0)
   {
      for(i = 0; i < nDim; i++)
         for(j = 0; j < nDim; j++)
            gsl_matrix_set(cluster->c->cov_matrix, (size_t)i, (size_t)j, 0.);
   ...
   for(i = 0; i < nDim; i++)
      for(j = 0; j < nDim; j++)
      {
         ...
         for(k = 0; k < cluster->nSamples; k++)
            cov += (gsl_vector_get(cluster->samples[k]->dns, (size_t)i) - gsl_vector_get(cluster->c->means, (size_t)i))*(gsl_vector_get(cluster->samples[k]->dns, (size_t)j) - gsl_vector_get(cluster->c->means, (size_t)j));
         cov /= (double)cluster->nSamples;

         if(fabs(cov) > 10E-10) nonZeroFlag = 1;
         gsl_matrix_set(cluster->c->cov_matrix, (size_t)i, (size_t)j, cov);
      }
...
void outIBIS(CLUSTER **clusters, int nClusters, int nDim, int convergedFlag)
{
   ...
   validClusters = (int*)calloc((size_t)nClusters, sizeof(int));
   nValidClusters = 0;
   for(i = 0; i < nClusters; i++)
   {
   ...
   zvmessage("========================================"," ");
   sprintf(message,"FINAL RESULT total num of clusters: %d converged: %s\n", nValidClusters, convergedFlag?"YES":"NO");
    zvmessage (message," ");
   zvmessage("========================================"," ");
   rowIndex = 0;
   for(i = 0; i < nClusters; i++)
   {
      ...
      // write means
      for(j = 0; j < nDim; j++)
         IBISHELPER_setDouble(out, index++, rowIndex, gsl_vector_get(clusters[i]->c->means, (size_t)j));

      // write cov matrix
      for(j = 0; j < nDim; j++)
         for(k = 0; k <= j; k++)
         {
            IBISHELPER_setDouble(out, index++, rowIndex, gsl_matrix_get(clusters[i]->c->cov_matrix, (size_t)j, (size_t)k));
   ...
   for(i = 0; i < nClusters; i++)
   {
      ...
      for(j = 0; j < clusters[i]->nSamples; j++)
      {
         ...
         //         printf("4 j: %d\n", j);
         for(k = 0; k < nDim; k++) IBISHELPER_setDouble(out, 3+k, rowIndex, gsl_vector_get(s->dns, (size_t)k));
         //         printf("5 j: %d\n", j);
         ++rowIndex;
      }
   }
...
//   IBISStruct *out;
   ...
   TMP *tmp;
    ...
    zvmessage ("clusterer - 10-09-2019 - rjb - 64-bit"," ");
   ...
   zvmessage("getting samples"," ");
   samples = readSamples(&nSamples, &nDim);
   zvmessage("getting temporary"," ");
   tmp = getTMP(nClusters, nDim);
   zvmessage("getting clusters"," ");
