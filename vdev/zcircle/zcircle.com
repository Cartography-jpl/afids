...
$! VPACK Version 1.9, Tuesday, August 16, 2016, 21:32:15
$!
...
$ vpack zcircle.com -mixed -
	-s zcircle.f -
	-i zcircle.imake -
	-p zcircle.pdf -
	-t tstzcircle.pdf tstzcircle.log
$ Exit
$ VOKAGLEVE
$ Return
$!#############################################################################
...
C************************************************************************
	...
	INTEGER*4 STATUS,NUMBER_OF_VALUES,PDF_DEFAULTED
...
C
        CALL IFMESSAGE('ZCIRCLE version 15-JULY-2016 - (64bit) - RJB')
        CALL XVEACTION('SA',' ')
...
C
	...
	IF ((SL.LT.1).OR.(SL.GT.IMG_NL)) THEN
	   CALL XVMESSAGE('??E - Invalid SL value',' ')
	   CALL ABEND
	ENDIF
	IF ((NL.LT.1).OR.(NL.GT.IMG_NL)) THEN
	   CALL XVMESSAGE('??E - Invalid NL value',' ')
	   CALL ABEND
	ENDIF
	IF ((SL+NL-1).GT.IMG_NL) THEN
	   CALL XVMESSAGE('??E - SL+NL-1 must be at most NL in image',' ')
	   CALL ABEND
	ENDIF
	IF ((SS.LT.1).OR.(SS.GT.IMG_NS)) THEN
	   CALL XVMESSAGE('??E - Invalid SS value',' ')
	   CALL ABEND
	ENDIF
	IF ((NS.LT.1).OR.(NS.GT.IMG_NS)) THEN
	   CALL XVMESSAGE('??E - Invalid NS value',' ')
	   CALL ABEND
	ENDIF
	IF ((SS+NS-1).GT.IMG_NS) THEN
	   CALL XVMESSAGE('??E - SS+NS-1 must be at most NS in image',' ')
	   CALL ABEND
	ENDIF
	...
	CALL XVPARM('R',R,NUMBER_OF_VALUES,PDF_DEFAULTED,1)
	   ...
	   RFLAG=.TRUE.
	   IF (R.LE.0) THEN
	      CALL XVMESSAGE('??E - Invalid radius value, reset to the ' //
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   ...
	   ELSE
	      CALL XVMESSAGE
     &             ('??E - DN for BYTE, HALF and FULL data format only',' ')
	      CALL ABEND
	   ENDIF
	ELSE
           IF (FMT.EQ.'BYTE') DN=0
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   ...
	   ELSE
	      CALL XVMESSAGE('??E - RDN for REAL data format only',' ')
	      CALL ABEND
	   ENDIF
	ELSE
	   RDN=DNMIN(4)
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   RPFLAG=.TRUE.
           IF (RPOL.LE.0) THEN
	      CALL XVMESSAGE
     &          ('??E - Invalid RPOL value, reset to the default value',' ')
	      RPFLAG=.FALSE.
	   ENDIF
	ENDIF
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   REFLAG=.TRUE.
           IF (REQ.LE.0) THEN
	      CALL XVMESSAGE
     &          ('??E - Invalid REQ value, reset to the default value',' ')
	      REFLAG=.FALSE.
	   ENDIF
	ENDIF
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   EFLAG=.TRUE.
	      ...
	      CALL XVMESSAGE
     &          ('??E - Invalid ECC value, reset to the default value',' ')
	      EFLAG=.FALSE.
	   ENDIF
	ENDIF
	...
	IF (PDF_DEFAULTED.EQ.0) THEN
	   AFLAG=.TRUE.
	      ...
	      CALL XVMESSAGE
     &          ('??E - Invalid ANG value, reset to the default value',' ')
	      AFLAG=.FALSE.
	   ENDIF
	ENDIF
C
...
C
	...
	IF (REFLAG.AND.RPFLAG.AND.EFLAG) THEN
	   CALL XVMESSAGE
     &            ('??E - ECC will be recalculated from REQ and RPOL',' ')
           ECC=SQRT(ABS(RPOL*RPOL-REQ*REQ))/(AMAX1(RPOL,REQ))
	ENDIF
...
C        
	NBYTES=int8(NS*PIX_SIZ)
C	CALL STACKA(ZMASK,1,NBYTES,NBYTES,&900)
	CALL STACKA_BIG(3,ZMASK,1,NBYTES)
C
C Close files
C
...
C	SUBROUTINE ZMASK(LINE_BUF,N1,NBYTES)
	...
	BYTE LINE_BUF(*)
        LOGICAL*1 INSIDE
	INTEGER*4 SL,SS,NL,NS,DN,DCODE,IN_UNIT,OUT_UNIT,PIX_SIZ
	INTEGER*4 I,STATUS,I1,I2,J,OFFSET
        INTEGER*8 N1
...
C
	   ...
	   ENDIF
           ...
           IF (INSIDE) GO TO 34
C
C Replace pixels before I1 (outside)
C
           IF (I1.GE.1) THEN
	      IF (DCODE.EQ.7) THEN
	   ...
	   ENDIF
C
C Replace pixels between I2 and NS (outside)
C
	   IF (I2.LE.NS)  THEN
...
GEN task completed
zcircle A B DN=10 RPOL=5. REQ=8. 'IN
Beginning VICAR task zcircle
ZCIRCLE version 15-JULY-2016 - (64bit) - RJB
  CENTER (X,Y) =    50    50
  RADII (EQU,POL) =  0.8000E+01  0.5000E+01
  AZIMUTH (NORTH) ANGLE =0.0000E+00
  ECCENTRICITY =0.7806E+00
list B (43 41 15 20)
Beginning VICAR task list

   BYTE     samples are interpreted as   BYTE   data
 Task:GEN       User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
 Task:ZCIRCLE   User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
     Samp    41      43      45      47      49      51      53      55      57      59
   Line
     43     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     44     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     45     100 100 100 100 100 100 100 100 100  10 100 100 100 100 100 100 100 100 100 100
     46     100 100 100 100  10  10  10  10  10  10  10  10  10  10  10 100 100 100 100 100
     47     100 100 100  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100 100 100
     48     100 100  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100 100
     49     100  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100
     50     100  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100
     51     100  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100
     52     100 100  10  10  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100 100
     53     100 100 100  10  10  10  10  10  10  10  10  10  10  10  10  10 100 100 100 100
     54     100 100 100 100  10  10  10  10  10  10  10  10  10  10  10 100 100 100 100 100
     55     100 100 100 100 100 100 100 100 100  10 100 100 100 100 100 100 100 100 100 100
     56     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     57     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
zcircle A B XCEN=-1 YCEN=20 RPOL=10 ECC=-0.2 'IN
Beginning VICAR task zcircle
ZCIRCLE version 15-JULY-2016 - (64bit) - RJB
  CENTER (X,Y) =    -1    20
  RADII (EQU,POL) =  0.9798E+01  0.1000E+02
  AZIMUTH (NORTH) ANGLE =0.0000E+00
  ECCENTRICITY =-.2000E+00
list B (1 1 30 10)
Beginning VICAR task list
 ...
 Task:ZCIRCLE   User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
   ...
   Line
     ...
     11       0   0   0 100 100 100 100 100 100 100
     12       0   0   0   0   0 100 100 100 100 100
     13       0   0   0   0   0   0 100 100 100 100
     14       0   0   0   0   0   0   0 100 100 100
     15       0   0   0   0   0   0   0 100 100 100
     16       0   0   0   0   0   0   0   0 100 100
     17       0   0   0   0   0   0   0   0 100 100
     18       0   0   0   0   0   0   0   0   0 100
     19       0   0   0   0   0   0   0   0   0 100
     20       0   0   0   0   0   0   0   0   0 100
     21       0   0   0   0   0   0   0   0   0 100
     22       0   0   0   0   0   0   0   0   0 100
     23       0   0   0   0   0   0   0   0 100 100
     24       0   0   0   0   0   0   0   0 100 100
     25       0   0   0   0   0   0   0 100 100 100
     26       0   0   0   0   0   0   0 100 100 100
     27       0   0   0   0   0   0 100 100 100 100
     28       0   0   0   0   0 100 100 100 100 100
     29       0   0   0 100 100 100 100 100 100 100
     30     100 100 100 100 100 100 100 100 100 100
zcircle A B ECC=-0.4 ANG=-45.0 RPOL=5. DN=50 'IN
Beginning VICAR task zcircle
ZCIRCLE version 15-JULY-2016 - (64bit) - RJB
  CENTER (X,Y) =    50    50
  RADII (EQU,POL) =  0.4583E+01  0.5000E+01
  AZIMUTH (NORTH) ANGLE =-.4500E+02
  ECCENTRICITY =-.4000E+00
list B (43 40 20 20)
Beginning VICAR task list
 ...
 Task:GEN       User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
 Task:ZCIRCLE   User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
     Samp    40      42      44      46      48      50      52      54      56      58
   Line
     43     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     44     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     45     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     46     100 100 100 100 100 100 100  50  50  50  50  50  50 100 100 100 100 100 100 100
     47     100 100 100 100 100 100  50  50  50  50  50  50  50  50 100 100 100 100 100 100
     48     100 100 100 100 100  50  50  50  50  50  50  50  50  50  50 100 100 100 100 100
     49     100 100 100 100 100  50  50  50  50  50  50  50  50  50  50  50 100 100 100 100
     50     100 100 100 100 100  50  50  50  50  50  50  50  50  50  50  50 100 100 100 100
     51     100 100 100 100 100  50  50  50  50  50  50  50  50  50  50  50 100 100 100 100
     52     100 100 100 100 100 100  50  50  50  50  50  50  50  50  50  50 100 100 100 100
     53     100 100 100 100 100 100 100  50  50  50  50  50  50  50  50 100 100 100 100 100
     54     100 100 100 100 100 100 100 100  50  50  50  50  50  50 100 100 100 100 100 100
     55     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     56     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     57     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     58     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     59     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     60     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     61     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
     62     100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100
zcircle A B R=4.
Beginning VICAR task zcircle
ZCIRCLE version 15-JULY-2016 - (64bit) - RJB
  CENTER (X,Y) =    50    50
  RADII (EQU,POL) =  0.4000E+01  0.4000E+01
  AZIMUTH (NORTH) ANGLE =0.0000E+00
...
Beginning VICAR task list
 ...
 Task:ZCIRCLE   User:rjb       Date_Time:Wed Jul 27 01:24:21 2016
   ...
   Line
     ...
     47       0   0   0   0   0   0   0   0 100 100 100 100 100   0   0   0   0   0   0   0
     48       0   0   0   0   0   0   0   0 100 100 100 100 100   0   0   0   0   0   0   0
     49       0   0   0   0   0   0   0 100 100 100 100 100 100 100   0   0   0   0   0   0
     50       0   0   0   0   0   0   0 100 100 100 100 100 100 100   0   0   0   0   0   0
     51       0   0   0   0   0   0   0 100 100 100 100 100 100 100   0   0   0   0   0   0
     52       0   0   0   0   0   0   0   0 100 100 100 100 100   0   0   0   0   0   0   0
     53       0   0   0   0   0   0   0   0 100 100 100 100 100   0   0   0   0   0   0   0
let $echo="no"
exit
slogoff
$ Return
