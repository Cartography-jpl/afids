...
int strncmp_caseless (char *str1, char *str2, int n);
...
long write_verify(int fh, char *source, long length, char *sErrorMessage);
...
void dump_str(char *name, char *source, int length);
void fDumpStr(FILE *fp, char *name, char *source, int length);
...
int find_text_tag(char *sFindTag, char *sText, char *sTagFound);
...
int write_screen_lines(int bReset, int max_lines, char *sLine);
...
DFParseType *ReadDefinitionFile(char *FileName);
...
char *DFPExportLoopCond(FILE *OutFp, char *Data, DFParseType *DFPStruct, char *parent_level_index, char *output_file);
...
int DFPExportData(char *out_filename, char *dff_filename, char *data, int bAutoIncrement, int data_len);
...
void loop_through_TREs(FILE *dest_fp, int image_num, int options, char *tre_to_dump);
...
void do_loop_through_TREs_to_file(FILE *fp, int image_num, char *sData, long total_length, int options, char *tre_to_dump);
