...
static double extractLat (char * mag, char * dir) {
  int deg, min, sec;
  double lat;
  char buf[10];
...
static double extractLon (char * mag, char * dir) {
  int deg, min, sec;
  double lon;
  char buf[10];
...
char * acqDate = 0, * oblAng = 0, * angleToNorth = 0, * sunEl = 0, * sunAz = 0, * meanGsd = 0, * dynamicRange = 0, * rollAng = 0;
char * mpd26a_6 = 0;
char * currentTRE = 0;
int rpcFieldCount = 0;
char ** rpcFieldNames = 0;
char ** rpcFieldValues = 0;
char * nitfCetag = 0;

static void dumpMeta (int image_num, char * metaPath, int nrows, int ncols) {
  char s_in[100];
  int i;
  FILE * metaData = fopen(metaPath, "w");

  if (iNITF_version == V2_0) {
    get_string(s_in, i20hdr[image_num].idatim, 14);
    fprintf (metaData, "NITF_ImageDatim=%s\n", s_in); /* GEOTIFF */
  } else if (iNITF_version == V2_1) {
    get_string(s_in, i21hdr[image_num].idatim, 14);
    fprintf (metaData, "NITF_ImageDatim=%s\n", s_in); /* GEOTIFF */
  }

  fprintf (metaData, "NITF_ROWS=%d\n", nrows); /* generic meta */
  fprintf (metaData, "NITF_COLUMNS=%d\n", ncols); /* generic meta */
  
  if (iNITF_version == V2_0)
    {
      get_string(s_in, i20hdr[image_num].igeolo, 60);
      /*      printf ("got V2_0 igeolo\n"); */
    }
  else if (iNITF_version == V2_1)
    {
      get_string(s_in, i21hdr[image_num].igeolo, 60);
      /*      printf ("got V2_1 igeolo\n"); */
    }

  /*    s_in[60] = '\0'; */
  /*    printf ("igeolo: %s\n", s_in); */

  if (iNITF_version == V2_0 || iNITF_version == V2_1) {
    fprintf (metaData, "NITF_CornerLat1=%.15lf\n", extractLat (&s_in[0],  &s_in[6])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLon1=%.15lf\n", extractLon (&s_in[7],  &s_in[14])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLat2=%.15lf\n", extractLat (&s_in[15], &s_in[21])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLon2=%.15lf\n", extractLon (&s_in[22], &s_in[29])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLat3=%.15lf\n", extractLat (&s_in[30], &s_in[36])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLon3=%.15lf\n", extractLon (&s_in[37], &s_in[44])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLat4=%.15lf\n", extractLat (&s_in[45], &s_in[51])); /* GEOTIFF */
    fprintf (metaData, "NITF_CornerLon4=%.15lf\n", extractLon (&s_in[52], &s_in[59])); /* GEOTIFF */
  }

  if (nitfCetag) {
    fprintf (metaData, "NITF_CETAG=%s\n", nitfCetag); /* GEOTIFF */
    nitfCetag = 0;
  }
  if (acqDate) {
    fprintf (metaData, "NITF_ACQ_DATE=%s\n", acqDate); /* GEOTIFF */
    acqDate = 0;
  }
  if (oblAng) {
    fprintf (metaData, "NITF_OBL_ANG=%s\n", oblAng); /* GEOTIFF */
    oblAng = 0;
  }
  if (angleToNorth) {
    fprintf (metaData, "NITF_ANGLE_TO_NORTH=%s\n", angleToNorth); /* GEOTIFF */
    angleToNorth = 0;
  }
  if (sunEl) {
    fprintf (metaData, "NITF_SUN_EL=%s\n", sunEl); /* GEOTIFF */
    sunEl = 0;
  }
  if (sunAz) {
    fprintf (metaData, "NITF_SUN_AZ=%s\n", sunAz); /* GEOTIFF */
    sunAz = 0;
  }

  if (meanGsd) {
    fprintf (metaData, "NITF_MEAN_GSD=%s\n", meanGsd); /* GEOTIFF */
    meanGsd = 0;
  }
  if (dynamicRange) {
    fprintf (metaData, "NITF_DYNAMIC_RANGE=%s\n", dynamicRange); /* GEOTIFF */
    dynamicRange = 0;
  }
  if (rollAng) {
    fprintf (metaData, "NITF_ROLL_ANG=%s\n", rollAng); /* GEOTIFF */
    rollAng = 0;
  }
  if (mpd26a_6) {
    fprintf (metaData, "NITF_MPD26A_FIELD6=%s\n", mpd26a_6); /* GEOTIFF */
    mpd26a_6 = 0;
  }

  for (i = 0; i < rpcFieldCount; i ++) {
    fprintf (metaData, "%s=%s\n", rpcFieldNames [i], rpcFieldValues [i]); /* GEOTIFF */
    free (rpcFieldNames [i]);
    free (rpcFieldValues [i]);
  }
  if (rpcFieldCount) {
    free (rpcFieldNames);
    free (rpcFieldValues);
...
void write_image_files(int image_types)
{
    ...
    for (image_num = 0; image_num < number_of_images; image_num++) {
	    ...
	    sscanf( i20hdr[image_num].nrows, "%8d", & nl);
	    sscanf( i20hdr[image_num].ncols, "%8d", & ns);
        }
        else if (iNITF_version == V2_1) {
            get_string(sIC, i21hdr[image_num].ic, 2);
            if (i21hdr[image_num].ic[0] == 'C' &&
                            i21hdr[image_num].ic[1] == '3') bJpeg = TRUE;
	    sscanf( i21hdr[image_num].nrows, "%8d", & nl);
	    sscanf( i21hdr[image_num].ncols, "%8d", & ns);
        }
	...
	{
	  char * outname = malloc( strlen( sOutputPath ) + 4 ); /* plus ".hlf" */
	  char * mvcmd = malloc (strlen (sFilename) + strlen( sOutputPath ) + 10);
	  extern int parmNL, parmNS;

	  if (parmNL > 0 && parmNS > 0) {
	    nl = parmNL;
	    ns = parmNS;
	  }

	  sprintf (outname, "%s.hlf", sOutputPath);
	  sprintf (mvcmd, "mv %s %s", sFilename, outname);
	  system (mvcmd);

	  sprintf (outname, "%s.txt", sOutputPath);
	  dumpMeta (image_num, outname, nl, ns);
	  free (outname);
	  free (mvcmd);
	}
...
/*      printf ("image_info[%d].nitf_file_offset: %d\n", image_num, image_info[image_num].nitf_file_offset); */
...
int unblock_image(int fh_in, int fh_out, int nbpp,
...
/*      static int firstWrite = 1; */
    ...
    if (r_pad < 0) {
        printf("unblock_image: Right padding width is negative %d pixels wide.\n", r_pad);
        iQuit(1);
    }
...
/*  		if (firstRead) { */
/*  		  firstRead = 0; */
/*  		  printf ("first pixel read: %d\n", ((short *) block[wblock_num].pdata)[0]); */
/*  		} */
...
/*  		    if (firstWrite) { */
/*  		      firstWrite = 0; */
/*  		      printf ("first pixel write: %d\n", ((short *) &block[wblock_num].pdata[y * block_width_bytes])[0]); */
/*  		    } */
