...
int DFPExportData(char *out_filename, char *dff_filename, char *data, int bAutoIncrement, int data_length)
{
    DFParseType *pt;
    DFParseType *DFPcur;
    FILE *fp;
...
*/
	    ...
	    {
	      /* from write_fs.c */
	      extern char * acqDate, * oblAng, * angleToNorth, * sunEl, * sunAz, * meanGsd, * dynamicRange, * rollAng;
	      extern char * mpd26a_6;

	      if (! strcasecmp (DFPcur->FieldName, "ACQ_DATE"))
		acqDate = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "ANGLE_TO_NORTH") ||
		       (currentTRE && ! strcmp (currentTRE, "use26a") && ! strcasecmp (DFPcur->FieldName, "FIELD1")))
		angleToNorth = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "MEAN_GSD") ||
		       (currentTRE && ! strcmp (currentTRE, "use26a") && ! strcasecmp (DFPcur->FieldName, "FIELD2")))
		meanGsd = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "DYNAMIC_RANGE") ||
		       (currentTRE && ! strcmp (currentTRE, "use26a") && ! strcasecmp (DFPcur->FieldName, "FIELD4")))
		dynamicRange = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "OBL_ANG") ||
		       (currentTRE && ! strcmp (currentTRE, "use26a") && ! strcasecmp (DFPcur->FieldName, "FIELD8")))
		oblAng = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "ROLL_ANG") ||
		       (currentTRE && ! strcmp (currentTRE, "use26a") && ! strcasecmp (DFPcur->FieldName, "FIELD9")))
		rollAng = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "SUN_EL") ||
		       (currentTRE && ! strcmp (currentTRE, "mpd26a") && ! strcasecmp (DFPcur->FieldName, "FIELD11")))
		sunEl = strdup (DFPcur->pFieldValue);
	      else if (! strcasecmp (DFPcur->FieldName, "SUN_AZ") ||
		       (currentTRE && ! strcmp (currentTRE, "mpd26a") && ! strcasecmp (DFPcur->FieldName, "FIELD10")))
		sunAz = strdup (DFPcur->pFieldValue);
	      else if (currentTRE && ! strcmp (currentTRE, "mpd26a") && ! strcasecmp (DFPcur->FieldName, "FIELD6"))
		mpd26a_6 = strdup (DFPcur->pFieldValue);
	    }
...
int DFPExportDataFp(FILE *dest_fp, char *dff_filename, char *data, int bAutoIncrement, int data_length)
{
    DFParseType *pt;
    DFParseType *DFPcur;
    char *CurData;
...
char *DFPExportLoopCond(FILE *OutFp, char *Data, DFParseType *p, char *parent_level_index, char *output_filename)
{
    DFParseType *DFPcur;
    DFParseType *DFPtemp;
    char *s;
...
char *DFPFillinField(DFParseType *p, char *data, char *output_filename)
{
    DFParseType *DFPTemp;
    int len;
    char sBuffer[1000];
...
void DFPPrintLine(FILE *fp, DFParseType *DFPcur, char *sLevelIndex)
{
    char sTemp[4096];
    extern char * currentTRE;
    char buf[100];

    extern int rpcFieldCount;
    extern char ** rpcFieldNames;
    extern char ** rpcFieldValues;
    extern char * nitfCetag;

    if (currentTRE && strstr (currentTRE, "rpc")) {

      if (! strcmp(currentTRE, "rpc00a"))
	nitfCetag = "RPC00A";
      else if (! strcmp(currentTRE, "rpc00b"))
	nitfCetag = "RPC00B";
      else
	nitfCetag = strdup(currentTRE);
	
      rpcFieldCount ++;
      rpcFieldNames = (char **) realloc (rpcFieldNames, sizeof (char *) * rpcFieldCount);
      rpcFieldValues = (char **) realloc (rpcFieldValues, sizeof (char *) * rpcFieldCount);
      sprintf (buf, "RPC_%s%s", DFPcur->FieldName, sLevelIndex);
      rpcFieldNames[rpcFieldCount - 1] = strdup(buf);
    }

    fprintf(fp, "%s%s = ", DFPcur->FieldName, sLevelIndex);
    if (DFPcur->IoConversionFn[0] != '\0') {
        HandleConversions(DFPcur, sTemp);
    }
    else
    {
    ...
    if (currentTRE && strstr (currentTRE, "rpc"))
      rpcFieldValues[rpcFieldCount - 1] = strdup(sTemp);
