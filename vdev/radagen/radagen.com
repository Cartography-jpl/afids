...
$! VPACK Version 1.9, Tuesday, August 16, 2016, 19:48:31
$!
...
$ vpack radagen.com -mixed -
	-s radagen.f -
	-i radagen.imake -
	-p radagen.pdf -
	-t tstradagen.pdf tstradagen.log
$ Exit
$ VOKAGLEVE
$ Return
$!#############################################################################
...
C
     ...
     +       REFUNIT, REF, UPD, MGN, RECTIFY, LOG, FILL, IFMT, OFMT
      REAL*4      REL, ILLUM_VECTOR(3), TAN_THETA, NADIR_OFFSET
      REAL*4      ALTI, RAZ, TOP_VALUE, HEIGHT, OUT_SCALE, LOG_SCALE
      REAL*4      AZ, EL, SCALE, SSCALE, LSCALE, ZSCALE, MIS_CORR(3)
      REAL*4 AZ_BUF(512),EL_BUF(512),SCALE_BUF(512)
      REAL*4 SSCALE_BUF(512),LSCALE_BUF(512),ZSCALE_BUF(512)
      REAL*4 HEIGHT_BUF(512),TOP_BUF(512)
      INTEGER*4   SS_BUF(512),NS_BUF(512),TOFF_BUF(512)
      INTEGER*2   OUT_SAMP, TOTAL_PIXELS_OUT, NEXT_SAMP
      INTEGER*4   MAXIM, TERRAIN_OFFSET, REFUNIT, LINC, IPASS
      INTEGER*4   ISL, ISS, NL, NS, INUNIT, OUTUNIT(2), N_OUTPUTS
      LOGICAL*4 REF, UPD, MGN, RECTIFY, LOG, FILL
      INTEGER*8 i8inp1,i8inp2,i8tmp 
      CHARACTER*5 IFMT, OFMT
C********************************************************************
C
      EXTERNAL WORK
C
      CALL IFMESSAGE('** RADAGEN version 27-JUL-2016 - (64bit) - RJB')
...
C
      ...
      i8inp2 = int8(NS*2)
      i8tmp  = int8(NS*4)
C      CALL STACKA(7,WORK,5,3*NS*2,NS*2,NS*2,NS*4,NS*4)
      CALL STACKA_BIG(7,WORK,5,i8inp1,i8inp2,i8inp2,i8tmp,i8tmp)
      RETURN
      END
C
C
...
C
     ...
     +	         INPUT,OUTPUT,TLOCATOR,SL)              !added TLOCATOR 7-27-2016
...
C
      CHARACTER*90 LBUF(2)
      INTEGER*4      ICNT, STATUS, MAXPARMS
      LOGICAL*4      XVPTST
C
C   OPEN THE INPUT
C
...
C
      REAL*4    PI/3.1415927/
      REAL*4    DEG_PER_RAD/57.29578/
      REAL*4    AUGMENTED_HEIGHT, R
      INTEGER*2 I
      CHARACTER*7200   WORK
C
      IF (NL.LT.3)
...
C
      ...
      INTEGER*2 I
      CHARACTER*7200   WORK
      INTEGER*4	PASS,BUF_CHECK
C
C  UPDATE THE PARAMETERS PER PASS
C
      BUF_CHECK = INT(SS_BUF(1))
      IF (BUF_CHECK.GE.PASS) ISS = SS_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) ISS = SS_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(NS_BUF(1))
      IF (BUF_CHECK.GE.PASS) NS = NS_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) NS = NS_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(AZ_BUF(1))
      IF (BUF_CHECK.GE.PASS) AZ = AZ_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) AZ = AZ_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(EL_BUF(1))
      IF (BUF_CHECK.GE.PASS) EL = EL_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) EL = EL_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(HEIGHT_BUF(1))
      IF (BUF_CHECK.GE.PASS) HEIGHT = 1000 * HEIGHT_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) HEIGHT = 1000*HEIGHT_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(TOP_BUF(1))
      IF ( BUF_CHECK.GE.PASS) TOP_VALUE = TOP_BUF(PASS)
      IF ( BUF_CHECK.LT.PASS) TOP_VALUE = TOP_BUF( BUF_CHECK)
C
      BUF_CHECK = INT(SSCALE_BUF(1))
      IF ( BUF_CHECK.GE.PASS) SSCALE = SSCALE_BUF(PASS)
      IF ( BUF_CHECK.LT.PASS) SSCALE = SSCALE_BUF( BUF_CHECK)
C
      BUF_CHECK = INT(LSCALE_BUF(1))
      IF (BUF_CHECK.GE.PASS) LSCALE = LSCALE_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) LSCALE = LSCALE_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(ZSCALE_BUF(1))
      IF (BUF_CHECK.GE.PASS) ZSCALE = ZSCALE_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) ZSCALE = ZSCALE_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(SCALE_BUF(1))
      IF (BUF_CHECK.GE.PASS) SCALE = SCALE_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) SCALE = SCALE_BUF(BUF_CHECK)
C
      BUF_CHECK = INT(TOFF_BUF(1))
      IF (BUF_CHECK.GE.PASS) TERRAIN_OFFSET = TOFF_BUF(PASS)
      IF (BUF_CHECK.LT.PASS) TERRAIN_OFFSET = TOFF_BUF(BUF_CHECK)
C
      IF (NL.LT.3)
     +   CALL XVMESSAGE(
     +    '?? W - RADAGEN INPUT MUST CONTAIN AT LEAST 3 LINES.',' ')
      TAN_THETA = TAN((90.-EL)/DEG_PER_RAD)*(SSCALE/ZSCALE) 
      NEXT_SAMP = NS - 1 
C
      REL = (EL/360.)*2*PI         ! CONVERT TO RADIANS. 
...
C
      ...
      INTEGER*2 INPUT(NS,3), OUTPUT(NS), TLOCATOR(NS)
C
      REAL*4    S(3), L(3), Z(3), Y(3), R3, R1, R2, TERRAIN_AVG
      INTEGER*2 SHADOW_HOLD(2)
      INTEGER*2 SHADOW_COUNT, SHADOW_LIMIT/2/
      INTEGER*2 SSS/1/, LL/2/, ALT/3/, TEMP
C      INTEGER*2 H/0/                                   !removed 7-27-2016
      INTEGER*4 I, II, LINE, SAMP, TOP, MIDDLE, BOTTOM, STATUS
C      LOGICAL        IN_RANGE
      CHARACTER*80   STRNG
C
     ...
     +			1,1,MIS_CORR)
      ...
      DO SAMP = NS-1,2,-1    ! UPPER LINE.
          IF (INPUT(SAMP,1).GE.TAN_THETA*(SAMP-NEXT_SAMP)) THEN
              ...
              CALL COMPUTE_FUNCTION(L,S,Z,ILLUM_VECTOR,Y,R1,R2,R3,SCALE,
     +				1,SAMP,MIS_CORR)                        !,R3)- Removed 7-27-2016
...
C=================================================================
      ...
      REAL*4 IS_CARTESIAN(3),EL,AZ,RAD
...
C********************************************************************
C
      REAL*4 L(3), S(3), Z(3), ILLUM_VECTOR(3), Y(3), R1, R2, R3, SCALE
      INTEGER*4  LINE, SAMP, REFUNIT
      REAL*4 DOTPROD, MIS_CORR(3)
...
C
      IMPLICIT NONE
      INTEGER*4	 LINE, SAMP
      REAL*4	 MIS_CORR(3), VALUE, VAL, PI/3.1415926536/
C
...
radagen inp=terrain.img out=rada.img
Beginning VICAR task radagen
** RADAGEN version 27-JUL-2016 - (64bit) - RJB

 OUTPUT RESOLUTION IN METERS/PIXEL          150.00
 AVG HEIGHT USED: 3129.00     LEFT BORDER OFFSET:   19
list rada.img
Beginning VICAR task list

   BYTE     samples are interpreted as   BYTE   data
 Task:FRACGEN   User:rjb       Date_Time:Wed Jul 27 01:09:43 2016
 Task:RADAGEN   User:rjb       Date_Time:Wed Jul 27 01:09:44 2016
     Samp     1       3       5       7       9
   Line
      1      77   7  14  22  29  37  44  52  59  67
      2     189 174 158 142 126 111  95  79  63  47
      3     192  10  14  18  22  26  30  34  38  42
      4     181 165 148 131 115  98  81  65  48  31
      5     160 139 118  97  75  54  33  11  19  27
      6     135  76  16  14  11  24  37  51  34  17
      7     254 175  96  17 254 205 156 107  58   9
      8     254 197 140  82  70  57  44  32  19   6
      9      90  33   0   0   0   0   0   0   0   0
     10       5   1   1   1   1   1   8   1   0   0
let $echo="no"
 
------------------------------------------------------------
!1 !! !! TEST MAGELLAN BACKSCATTER MODEL
                    AND 
         INTERNAL RECTIFICATION ADDITIONS !! !! !! 
------------------------------------------------------------
...
Beginning VICAR task list
 ...
 Task:RADAGEN   User:rjb       Date_Time:Wed Jul 27 01:09:44 2016
   ...
   Line
      1     13136 13136 13136 13136 13136 13136 13136 13136 13136 13136
      2     14630  3914  3338 14630 14630 14630 14630 14630 14630 14630
      3     14669  3338  4676 14669 14669 14669 14669 14669 14669 14669
      4     14308  3338  3338 14308 14308 14308 14308 14308 14308 14308
      5     14029  3338 14029 14029 14029 14029 14029 14029 14029 14029
      6     14786  3338 14786 14786 14786 14786 14786 14786 14786 14786
      7     16383 16383 16383 16383 16383 16383 16383 16383 16383 16383
      8     16383 16383 16383 16383 16383 16383 16383 16383 16383 16383
      9     12466 12466 12466 12466 12466 12466 12466 12466 12466 12466
     10      3338  3338  3338  3338  3338  3338  3338  3338  5291  5291
let $echo="no"
...
** RADAGEN version 27-JUL-2016 - (64bit) - RJB
 ...
 OUTPUT RESOLUTION IN METERS/PIXEL           19.69
 AVG HEIGHT USED:    0.00     LEFT BORDER OFFSET:    0
list radd.img
Beginning VICAR task list
 ...
 Task:RADAGEN   User:rjb       Date_Time:Wed Jul 27 01:09:44 2016
     Samp     1       3       5       7       9
   Line
      1      65  65  65  65  65  65  65  65  65 254
      2     254 254 254 254 254 254 254 254 254 254
      3     254 254 254 254 254 254 254 254 254 254
      4     254 254 254 254 254 254 254 254 254 254
      5     254 254 254 254 254 254 254 254 254 254
      6     254 254 254 254 254 254 254 254 254 254
      7     254 254 254 254 254 254 254 254 254 254
      8     254 254 254 254 254 254 254 254 254 254
      9     254 254 254 254 254 254 254 254 254 254
     10       1   1   1   1   1   1   1   1 101 201
let $echo="no"
exit
slogoff
$ Return
