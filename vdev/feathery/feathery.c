...
void main44(void)
{
   ...
   int ixl,ixr,ibj,nseq,nincr,*firstread,bix,ibix,nbrseq;
   ...
   int geotiff,labnl,labns,tolerct,tolerdf,img,rot1,rot2;
   int sctype1,sctype2,*ilcorner,*iscorner,*inl,*ins,isav,moorenbl;
   int igncase,divix,wfeather,biv[50],itmp;
   int **gj,**gi,**giout;
   int bix0,bix1,gcount0,gcount1,gfeather,tlistix;
   short int ***footprnt,*bufout,*bufhit,pself,pnbr,i2dkthr=0;
   short int **inbuf,***moore,mooretst=0,mnbr,*gptr;
   ...
   double *mffac1,**sumz,ztot,mmp,divmax,dtmp;
   float vi,usum,di,**dz,*zbar,gorefac,vivsave,**ggrey,gsum;
   float zramp=0,vself,vnbr,hnl,hns,locnl,locns,**ibisdz,fbigthresh=0;
   float vdiff,div[50],viv[50],gcor0,gcor1,ggrey0,ggrey1,viv0c,viv1c,gcon;
   float greyfac;
   
   /* initialize, fetch params */

   zifmessage("feathery version Thu Oct 09 2008");
   ...
   zvp("wfeather",&wfeather,&dummy);
   zvp("gfeather",&gfeather,&dummy);
      ...
      for (j=1;j<filcount;j++)
         ...
         if (openstat[j]==0) /* check for top line and open */
            ...
            if (mfsl[ibj]<=(sline+iread))
               ...
               if (dfeather<tempnl/2&&dfeather<tempns)
                  zmabend("dfeather too small for inputs");
   ...
   tlistix = 0; vivsave = 0.0; gcon = 0.0;
   ...
   mz_alloc2((unsigned char ***)&gj,maxcross,10,4);
   mz_alloc2((unsigned char ***)&gi,maxcross,10,4);
   mz_alloc2((unsigned char ***)&giout,maxcross,10,4);
   mz_alloc2((unsigned char ***)&ggrey,maxcross,10,4);
   mz_alloc1((unsigned char **)&gptr,maxcross,2);
   for (i=0;i<maxcross;i++) gptr[i] = 0;

   for (iread=1-dfeather,iout=2-2*dfeather;iout<nline;iread++,iout++)
      {
      if (progress&&iout%1000==-999) printf(".");
      if (progress&&iout%500==499)
      ...
      for (j=1;j<filcount;j++)
         ...
         if (openstat[j]==0) /* check for top line and open */
            ...
            if (mfsl[ibj]<=(sline+iread))
               ...
               zvget(vunit[j],"NL",&tempnl,NULL);
               zvget(vunit[j],"NS",&tempns,NULL);
               if (dfeather<tempnl/2&&dfeather<tempns/2)
                  zmabend("dfeather too small for inputs");
               if (tempnl!=mfnl[ibj]||tempns!=mfns[ibj])
                  {
                  printf("file %s has\n",infil[j-1]);
         ...
         for (i=0;i<nsamp;i++)
            ...
            if (listix==1)
               ...
               if (i%10==0)
                  ...
                  gj[bix][gptr[bix]] = j;
                  gi[bix][gptr[bix]] = i;
                  giout[bix][gptr[bix]] = iout;
                  ggrey[bix][gptr[bix]++] = (float)bufout[i];
                  if (gptr[bix]==10) gptr[bix] = 0;
                  }
               }
               ...
               if (tlistix==1)
                  {
                  bufout[i] = (short int)(vivsave+0.5);
                  goto tlistix1;
                  }
               if (listix>1)
                  {
                  divmax = -999.0; divix = -1;
                  for (k=0;k<listix;k++)
                     if (div[k]>divmax&&viv[k]>0.0) { divmax = div[k]; divix = k; }
                  if (divix>-1)
                     {
                     dtmp = div[0]; div[0] = divmax; div[divix] = dtmp;
                     dtmp = viv[0]; viv[0] = viv[divix]; viv[divix] = dtmp;
                     itmp = biv[0]; biv[0] = biv[divix]; biv[divix] = itmp;
                     }
                  divmax = -999.0; divix = -1;
                  for (k=1;k<listix;k++)
                     if (div[k]>divmax&&viv[k]>0.0) { divmax = div[k]; divix = k; }
                  if (divix>-1)
                     {
                     dtmp = div[1]; div[1] = divmax; div[divix] = dtmp;
                     dtmp = viv[1]; viv[1] = viv[divix]; viv[divix] = dtmp;
                     itmp = biv[1]; biv[1] = biv[divix]; biv[divix] = itmp;
                     }
                  }

               bix0 = biv[0]; bix1 = biv[1];
               mmp = div[0]-div[1];

               if (fabs(mmp)>(double)gfeather)
                  { 
                  if (mmp>0) usum = viv[0]; else usum = viv[1];
                  }
               else
                  {
                  greyfac = 1.0-fabs(mmp)/fabs(gfeather);
                  /*obtain and verify grey avgs*/
                  gsum = 0.0; gcount0 = 0;
                  for (k=0;k<10;k++)
                     {
                     if (gj[bix0][k]!=gj[bix0][0]) break;
                     if (gi[bix0][k]<i-105||gi[bix0][k]>i) break;
                     if (giout[bix0][k]!=iout) break;
                     gsum += ggrey[bix0][k];
                     gcount0++;
                     }
                  ggrey0 = 0.1*gsum*greyfac;

                  gsum = 0.0; gcount1 = 0;
                  for (k=0;k<10;k++)
                     {
                     if (gj[bix1][k]!=gj[bix1][0]) break;
                     if (gi[bix1][k]<i-105||gi[bix1][k]>i) break;
                     if (giout[bix1][k]!=iout) break;
                     gsum += ggrey[bix1][k];
                     gcount1++;
                     }
                  ggrey1 = 0.1*gsum*greyfac;
                  
                  if (gcount0*gcount1==100)
                     {
                     gcon = (gfeather+mmp)/(2.0*gfeather);
                     gcor0 = (1.0-gcon)*(ggrey1-ggrey0);
                     gcor1 = gcon*(ggrey0-ggrey1);
                     }
                  else { gcor0 = 0.0; gcor1 = 0.0; }
                  
                  viv0c = viv[0]+gcor0;
                  viv1c = viv[1]+gcor1;
                  if (fabs(mmp)>wfeather)
                     { if (gcon<0.5) usum = viv1c; else usum = viv0c; }
                  else
                     usum = ((wfeather+mmp)*viv0c+(wfeather-mmp)*viv1c)/(2.0*wfeather);
                  }
               
               if (div[0]<0.01&&div[1]<0.01) bufout[i] = (short int)0;
               else if (adjust!=3) 
                  bufout[i] = MAX((short int)0,(short int)(usum+0.5));
               else 
                  bufout[i] = MAX((short int)1,(short int)(usum+0.5));
               } /* listix>1 case */
            tlistix1:
