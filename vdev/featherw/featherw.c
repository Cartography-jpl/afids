...
void leftprop(ival,moore_bix,currix,i,redtns)
   int ival,currix,i,redtns;
   short int **moore_bix;
{
   int ptr,tp,ti,tv;
...
/* used only by feather, do not move to library */
void edgeops(footprntline,lnibble,rnibble,lthresh,rthresh,redtns,nincr,nseq)
   short int *footprntline;
   int lnibble,rnibble,lthresh,rthresh,redtns,nincr,nseq;
{
   int i,k,thrcount;
   
   for (i=0;i<lnibble;i++) footprntline[i] = 0;
   for (i=1;i<=rnibble;i++) footprntline[redtns-i] = 0;
   if (lthresh>0)
      {
      thrcount = 0;
      for (i=lnibble;i<redtns;i+=nincr)
         {
         if (footprntline[i]>=lthresh)
            {
            thrcount++;
            if (thrcount>=nseq)
               {
      ...
      if (thrcount<nseq) 
         {
         for (k=lnibble;k<redtns;k++) footprntline[k] = 0;
         return;
         }
      }
   if (rthresh>0)
...
/* used only by feather, do not move to library */
void bigprop(footprntline,moorebix,redtns,thresh,currix,previx,
       redfeather,interior)
   short int *footprntline;
   short int **moorebix;
   int redtns,thresh,currix,previx,redfeather,interior;
{
   int i,datafound,leftbrk=0,rightbrk=0,midns,prevmoore,newmoore;
   ...
   if (interior)
      ...
      for (i=0;i<redtns;i++)
         {
         if (footprntline[i]>thresh)
            {
            newmoore = MIN(redfeather,MIN(prevmoore,moorebix[previx][i])+1);
            }
         else
            {
            newmoore = 0;
            if (moorebix[previx][i]>1)
               propagate(1,moorebix,previx,i,redfeather,redtns,currix);
            if (prevmoore>1)
               leftprop(1,moorebix,currix,i-1,redfeather,redtns);
            }
         moorebix[currix][i] = newmoore;
         prevmoore = newmoore;
         }
      }
   else
      ...
      for (i=0;i<redtns;i++)
         {
         leftbrk = i;
         if (footprntline[i]>=thresh) { datafound = 1; break; }
         moorebix[currix][i] = (short int)0;
         if (previx>=0&&((int)(moorebix[previx][i])>1))
            propagate(1,moorebix,previx,i,redfeather,redtns,currix);
         }
      if (!datafound) return;
      for (i=redtns-1;i>=0;i--)
         {
         rightbrk = i;
         if (footprntline[i]>=thresh) break;
         moorebix[currix][i] = (short int)0;
         if (previx>=0&&((int)(moorebix[previx][i])>1))
            propagate(1,moorebix,previx,i,redfeather,redtns,currix);
         }
      midns = (leftbrk+rightbrk)/2;
      if (leftbrk==0) moorebix[currix][leftbrk++] = (short int)1;
      if (rightbrk==(redtns-1)) moorebix[currix][rightbrk--] = (short int)1;
      for (i=leftbrk;i<midns;i++)
         {
         moorebix[currix][i] = (short int)MIN((int)moorebix[currix][i-1]+1,
            MIN((int)moorebix[previx][i]+1,redfeather));
         }
...
void main44(void)
{
   ...
   int cols[7],inlist[50],sampoff[50],progress,tval;
   ...
   int geotiff,labnl,labns,tolerct,tolerdf,img,rot1,rot2,interior;
   ...
   zifmessage("featherw version Fri Jan 11 2008");
   ...
   redmfns = 100+(maxmfns+moorefac-1)/moorefac; /*see featherv alz */
   ...
   for (iread=1-dfeather,iout=2-2*dfeather;iout<nline;iread++,iout++)
      ...
      for (j=1;j<filcount;j++)
         ...
         if (openstat[j]==1) /* read the line and apply Moore */
            ...
            if (edge) edgeops(footprnt[bix][currix],lnibble,rnibble,
                  lthresh,rthresh,redtns,nincr,nseq);
            ...
            bigprop(footprnt[bix][currix],moore[bix],redtns,thresh,currix,
                      previx,redfeather,interior);
