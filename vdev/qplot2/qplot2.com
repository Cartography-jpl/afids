...
$! VPACK Version 1.9, Tuesday, August 16, 2016, 19:44:05
$!
...
C
	...
	integer*4 i,ii,j,jj,n,icount,idef,iline,isize,psize
...
c
      call xvmessage('qplot2 version 28-Aug-2013 (64-bit gnuplot) - rjb',' ')
C
...
c	print *, 'before CALL STACKA(9,EQUIV,.... tmptbl = ',tmptbl(1:ntbl)   
        LX8 = int8(LX)
        LY8 = int8(LY) 
         CALL STACKA_BIG(9,EQUIV,2,LX8,LY8,LX,LY,iline,tmptbl,ntbl)
c         IF (IND .EQ. 1) GO TO 999
...
c999   CALL XVMESSAGE('??E - Stacka error',' ')
c      CALL ABEND
      END
C
C  **********************************************************
C
      SUBROUTINE EQUIV(X,LX8,Y,LY8,LX,LY,LINE,tmptbl,ntbl)
...
C
	integer*4 lx,ly
	integer*4 line,ntbl
        integer*8 lx8,ly8,dum
	real*4 x(lx),y(ly)
	character*24 tmptbl
c
	dum=lx8		!to suppress warning msg in compiler
        dum=ly8
...
HISTORY:
  ...
  08-21-2013 - Ray Bambery - Fixed grid, notics code in gpi output. Fixed
                        DISPLACE parmeter, Changed mode of placing labels
                        on plots from proportional to character mode. 
                        Added filenames to plot
  08-26-2013 - Ray Bambery - Added NOLABEL parameter to inhibit overprinting
