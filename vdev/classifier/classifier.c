...
{
   gsl_vector *dns;
   int i, j, status,inpCnt, outCnt, nClasses, isExpMax, gnu_errno;
//  int  dumdef, dumcnt;
   double *dist;
   ...
   CLASS_TMP *tmp;
    ...
    status=0;
    zvmessage ("classifier - 10-17-2019 - rjb - 64-bit:"," ");
   isExpMax = zvptst("EXPMAX");
   sprintf(message,"isExpMax: %d\n", isExpMax);
    zvmessage(message," ");
   status = zvpcnt("inp", &inpCnt);
   if (status != 1) {
        zvmessage ("zvpcnt status != 1"," ");
        zmabend ("ABEND");
    }
   inps = (VICAR_IMAGE**)malloc(sizeof(VICAR_IMAGE*)*(long unsigned int)(inpCnt));
   ...
   nClasses = ibis->nr;
   dist = (double*)malloc(sizeof(double)*(long unsigned int)nClasses);
   ...
   dns = gsl_vector_alloc((size_t)inpCnt);
   for(i = 0; i < inps[0]->nl; i++)
   {
      ...
      for(j = 0; j < inps[0]->ns; j++)
      {
         ...
         for(k = 0; k < inpCnt; k++) 
            gsl_vector_set(dns, (size_t)k, inps[k]->buffer[j]);
