...
#endif
/* prototypes */
void propagate(int ival,short int **moorebuf,int previx,int i,int dfeather,
    int tns,int currix);
void leftprop(int ival,short int **moorebuf,int currix,int i);
...
void leftprop(ival,moorebuf,currix,i)
   int ival,currix,i;
   short int **moorebuf;
{
   int ptr,tp,ti,tv;
...
void main44(void)
{
   ...
   zifmessage("mooresc 10-06-2019 - rjb - 64-bit");
   ...
   for (i=0;i<numpix;i++) lookup[pixval[i]] = (short int)(i+1);
         ...
         for (j=0;j<ns;j++)
            ...
            if (inbuf[ip][currix][j]==0)
               ...
               if ((prevmoore>newmoore+1)&&j>0)
                  leftprop(newmoore+1,moore[ip],currix,j-1);
               }
