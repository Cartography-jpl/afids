...
/* prototype */
void sortrec7(float *key,int *ptr,int len);
   ...
   float *key;
{
   ...
   if (len<2) return;
   if ((temp=(float *)malloc(4*(size_t)len))==NULL)
...
void main44(void)
{
   ...
   int ounit1,ounit2,ounit3,filcount,fildef,ier,status,vsize[4],mode,ntot;
   int *dnsort,inverse;
   short int **buf;
   ...
   zifmessage("calfit 10-17-2019 - rjb - 64-bit");
   ...
   for (i=0;i<inpcnt;i++)
      ...
      if (status != 1) {
        sprintf (message,"status = %d for zvunit on %d\n",status,vunit[i]);
        zvmessage(message," ");
        zmabend("ABEND");
      }
      status = zvopen(vunit[i],"OPEN_ACT","SA",
                 "IO_ACT","SA","U_FORMAT","HALF", NULL);
       if (status != 1) {
        sprintf (message,"status = %d for zvopen on %d\n",status,vunit[i]);
        zvmessage(message," ");
        zmabend ("ABEND");
       }
   ...
   status=zvopen(ounit1,"U_NL",vsize[2],"U_NS",vsize[3],"U_FORMAT","REAL",
   ...
   status=zvopen(ounit2,"U_NL",vsize[2],"U_NS",vsize[3],"U_FORMAT","REAL",
	"OP","WRITE","OPEN_ACT","SA","IO_ACT","SA","O_FORMAT","REAL", NULL);
      if (status != 1) {
        sprintf (message,"status = %d for zvopen on %d\n",status,ounit2);
        zvmessage(message," ");
   ...
   if (filcount==3)
     ...
     if (status != 1) {
      ...
      status=zvopen(ounit3,"U_NL",vsize[2],"U_NS",vsize[3],"U_FORMAT","REAL",
	   "OP","WRITE","OPEN_ACT","SA","IO_ACT","SA","O_FORMAT","REAL", NULL);
             if (status != 1) {
        sprintf (message,"status = %d for zvopen on %d\n",status,ounit3);
        zvmessage(message," ");
   ...
   for (iout=0;iout<vsize[2];iout++)
      ...
      for (jout=0;jout<inpcnt;jout++)
         {
         status = zvread(vunit[jout],buf[jout],
      ...
      for (k=0;k<vsize[3];k++)
         ...
         if (ier!=0)
            ...
            {
            outa[iout][k] = (float)csol[0];
            outb[iout][k] = (float)csol[1];
            if (filcount==3) outq[iout][k] = (float)csol[2];
            }
         }
      }
   ...
   if (mode==0)
      ...
      for (iout=0;iout<vsize[2];iout++)
         ...
         for (k=0;k<vsize[3];k++)
            {
            dn0[ptr] = (float)(outa[iout][k]*calval[0]+outb[iout][k]);
            dnn[ptr++] = (float)(outa[iout][k]*calval[inpcnt-1]+outb[iout][k]);
            }
         }
