...
void open_files(int* filetype)
{
  ...
  /* open the input file */
  /***********************/
  if (zvunit( &i_unit, "INP", 1, NULL) != 1)
    zmabend("zvunit failed on INP ");
  if (zvopen( i_unit, "OPEN_ACT", "SA", "IO_ACT", "SA", NULL) != 1)
    zmabend("zvopen failed on OUT");

  /* Make sure we have either BYTE or HALF */
  zvget(i_unit,"FORMAT",fmt_str, NULL);
  if ( strcmp(fmt_str,"BYTE") && strcmp(fmt_str,"HALF") && strcmp(fmt_str,"FULL") && strcmp(fmt_str,"REAL") && strcmp(fmt_str,"DOUB")) {
    zvmessage("Invalid input data format.  Use BYTE, HALF, FULL, REAL, or DOUB.","");
    zabend();
  }
  if (strcmp(fmt_str,"BYTE")==0)
    ...
    *filetype = 2;
  else if (strcmp(fmt_str,"REAL")==0)
    *filetype = 3;
  else if (strcmp(fmt_str,"DOUB")==0)
    *filetype = 4;
  else
    zmabend("unsupported type");
  ...
  if (zvunit( &o_unit, "OUT", 1, NULL) != 1)
    zmabend("zvunit failed on OUT");

  /* note that zvopen is intelligent enough to default to the same */
  /* format as the input file.  */
  if (zvopen( o_unit, "U_NL", nline, "U_NS", nsamp,
	      "OP", "WRITE",
	      "OPEN_ACT", "SA",
	      "IO_ACT", "SA", NULL) != 1)
    zmabend("zvopen failed on OUT");
...
void get_extrema(int newsl, int newss, int newnl, int newns, int nahm1, int navm1, int grids, double ldel, double sdel,
		 double* outl, double* outs, double* inl, double* ins, int jtop, double* lmin, double* lmax, double* smin, double* smax)
{
...
template<typename T>
void process_block(int sl, int ss, int nl, int ns, int nlp6, int nsp6, int newsl, int newss,
		   int newnl, int newns, double* outl, double* outs, double* inl, double* ins, int jtop, int nahm1, int navm1, int grids, int interp,
		   int znoin, double ldel, double sdel, int istrip, FILE* fileptr, int newnswrite)
{
   int i,j,iline,bx,cxl,cxs,nloop,tin,xl,xs,outcount,ipos,jpos;
   int icell,jcell,bufpos,jgrp,cubtype;
   ...
   mz_alloc2((unsigned char ***)&buf,nlp6,nsp6,sizeof(T));
   mz_alloc1((unsigned char **)&outbuf,newnswrite,sizeof(T));
   for (i=1;i<=nl;i++)
     {
       if (zvread(i_unit,&buf[i+1][2],"LINE", i+sl,
		  "SAMP", ss+1, "NSAMPS", ns, NULL) != 1)
	 zmabend("zvread failed");
       buf[i+1][0] = 0;
       buf[i+1][1] = 0;
       buf[i+1][ns+2] = 0;
       buf[i+1][ns+3] = 0;
...
case 1:           rl = fxl1*buf[cxl][cxs]+fxl*buf[cxl+1][cxs];
	          ...
	          outbuf[bufpos+j] = (T)(fxs1*rl+fxs*ru+.5);
                  break;
...
case 2:           /* cubic convolution */
                  ...
                  outbuf[bufpos+j] = (T)(c1*r1+c2*r2+c3*r3+c4*r4+.5);
                  break;
	 ...
	 bufpos += nloop;
      ...
      if (istrip<0)
	   ...
	   outcount++;
         ...
         {
	   fwrite(outbuf,sizeof(T),newnswrite,fileptr);
         }
      }
...
void main44(void)
{
   ...
   zifmessage("geomv version 2018-10-02");
   
   open_files(&filetype);

   switch(filetype) {
   case 0:			// byte
     writepix = sizeof(char); break;
   case 1:			// half
     writepix = sizeof(signed short int); break;
   case 2:			// full
     writepix = sizeof(signed int); break;
   case 3:			// real
     writepix = sizeof(float); break;
   case 4:			// doub
     writepix = sizeof(double); break;
   default:
     writepix = 0;		// silence uninit-var-used compiler warning
     zmabend("unsupported type");
   }
   ...
   if (parmcnt>1)
      ...
      status = IBISColumnSet(ibis,ICOLUMN_U_FORMAT,(void*)"DOUB",cols[0]);
      status = IBISColumnSet(ibis,ICOLUMN_U_FORMAT,(void*)"DOUB",cols[1]);
      status = IBISColumnSet(ibis,ICOLUMN_U_FORMAT,(void*)"DOUB",cols[2]);
      status = IBISColumnSet(ibis,ICOLUMN_U_FORMAT,(void*)"DOUB",cols[3]);
   ...
   if (lbufsiz>(long int)vmemsize)
      ...
      for (istrip=0;istrip<stripn;istrip++)
	   ...
	   printf("strip %d/%d\n", istrip, stripn);
         ...
         for (jstrip=0;jstrip<stripn;jstrip++)
	      ...
	      switch(filetype) {
	      case 0:		// byte
		process_block<unsigned char>(sl,ss,nl,ns,nlp6,nsp6,snewsl,snewss,
					     snewnl,snewns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
					     znoin,ldel,sdel,istrip,fileptr,snewnswrite);
		break;
	      case 1:		// half
		process_block<signed short int>(sl,ss,nl,ns,nlp6,nsp6,snewsl,snewss,
						snewnl,snewns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
						znoin,ldel,sdel,istrip,fileptr,snewnswrite);
		break;
	      case 2:		// full
		process_block<signed int>(sl,ss,nl,ns,nlp6,nsp6,snewsl,snewss,
					  snewnl,snewns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
					  znoin,ldel,sdel,istrip,fileptr,snewnswrite);
		break;
	      case 3:		// real
		process_block<float>(sl,ss,nl,ns,nlp6,nsp6,snewsl,snewss,
				     snewnl,snewns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
				     znoin,ldel,sdel,istrip,fileptr,snewnswrite);
		break;
	      case 4:		// doub
		process_block<double>(sl,ss,nl,ns,nlp6,nsp6,snewsl,snewss,
				      snewnl,snewns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
				      znoin,ldel,sdel,istrip,fileptr,snewnswrite);
		break;
	      default:
		zmabend("unsupported filetype");
	      }
	    }
            }
         rewind(fileptr);
      ...
      switch(filetype) {
      case 0:			// byte
	process_block<unsigned char>(sl,ss,nl,ns,nlp6,nsp6,newsl,newss,
				     newnl,newns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
				     znoin,ldel,sdel,-1,0,newns);
	break;
      case 1:			// half
	process_block<signed short int>(sl,ss,nl,ns,nlp6,nsp6,newsl,newss,
					newnl,newns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
					znoin,ldel,sdel,-1,0,newns);
	break;
      case 2:			// full
	process_block<signed int>(sl,ss,nl,ns,nlp6,nsp6,newsl,newss,
				  newnl,newns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
				  znoin,ldel,sdel,-1,0,newns);
	break;
      case 3:			// real
	process_block<float>(sl,ss,nl,ns,nlp6,nsp6,newsl,newss,
			     newnl,newns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
			     znoin,ldel,sdel,-1,0,newns);
	break;
      case 4:			// doub
	process_block<double>(sl,ss,nl,ns,nlp6,nsp6,newsl,newss,
			      newnl,newns,outl,outs,inl,ins,jtop,nahm1,navm1,grids,interp,
			      znoin,ldel,sdel,-1,0,newns);
	break;
      default:
