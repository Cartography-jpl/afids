...
void main44(void)
{
    ...
    int inptr[48],tpixsiz=0,status,dummy,icomp,unit2,ibis2,ncol2,clen2;
    ...
    zifmessage("comptab2 version Thu Nov  6 2017");
  
    compress_flag = 0;          /* no compressed images */
  /* get some parms */
    ...
    for (i=0;i<inpcnt;i++) {
        ...
        /* check if compressed image */
        status=zlget(vunit[i],"SYSTEM","compress",value,"ERR_ACT","SA",
         NULL);

        
        if ((strcmp(value,"BASIC") == 0 ) || (strcmp(value,"BASIC2") == 0  )) {
            sprintf( msg, " input image %d - %s compression\n", i+1, value );
            zvmessage (msg," "); 
            compress_flag++;
        }
      ...
      /* resolve input samples */
        if (vsize[3]!=0) {
            outnsamp = MIN(vsize[3],ins[i]-vsize[1]+1);
        } else {
            outnsamp = ins[i];
        }
      ...
      /* resolve input lines */
        if (vsize[2]!=0) {
            outnline = MIN(vsize[2],inl[i]-vsize[0]+1);
        } else {
            outnline = inl[i];
        }
        ...
        if (i==0) {
	        ...
	        if (inl[i]!=bignline) zmabend("??E - Images must have same NL");
    ...
    if (compress_flag > 0) {
        zvmessage ("??E - One or more of images are compressed"," "); 
        zmabend ("??E - Compressed images are illegal for comptab2");
    }    
    ...
    if (!inbuf || !stat || !centroidL || !centroidS) {
        zmabend( "??E - buffer allocation failed");
    }
  ...
  /* process the statistics */
    ...
    printf ("ncol2 = %d\n",ncol2);
    for (i=0;i<4;i++) {
        strcpy(cformat[i],"FULL");
          printf ("cformat[i] =  %d  %s\n",i,cformat[i]);   
    }
    for (i=4;i<ncol2;i++) {
        strcpy(cformat[i],"DOUB");
          printf ("cformat[i] = %d  %s\n",i,cformat[i]);    
    }
 ...
 for (i=0;i<4;i++)
  ...
  /*status = IBISFileOpen(ibis2,&ibis,"write",4,clen,0,0);*/
    ...
    status = IBISColumnWrite(ibis2, (char*)centroidL, 6, 1, clen2);     //6
    if(status != 1) IBISSignal(ibis2, status, 1);
    printf ("i = 6\n");
    status = IBISColumnWrite(ibis2, (char*)centroidS, 7, 1, clen2);     //7
    if(status != 1) IBISSignal(ibis2, status, 1);
    printf ("i = 7\n");
    printf ("i+6 = %d\n",i+6);
