 ...
 **
 **    And:
 **      Special (patented) JPL/NGT compression codec libraries.
...
typedef unsigned short u_short;alz*/
...
/* I defined OLD_COMPRESSION_CODE
   to enable LZW compression.  pk */
...
{   
  ...
  zifmessage("vtiff3 version Thu Jan  3 2008");
...
#ifdef HAS_PRINT_HANDLER
  TIFFSetPrintHandler( PrintHandler );
#endif
...
/*    printf ("adding gtlabel key=%s value=%s\n", key, value); */
...
/*        printf ("exporting %d geo tiff tiepoints\n", doubleCount / 6); */
  ...
  /* From BigMap WriteTiff.c:
    	sprintf(loc_string, 
    	"CART_RESOURCE(%d,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu)", 
    	 0, gLongitude, gLatitude,  
    	 gPixDegLonInt, gPixDegLonFract, 
    	 gPixDegLatInt, gPixDegLatFract,  
    	 gXPixel, gYPixel); 
    	TIFFSetField(tifFile, TIFFTAG_IMAGEDESCRIPTION,loc_string); 

     Example: CART_RESOURCE(0,54000000,576000000,762,1336212047,2224,1527099482,2112,6144)

     From BigMapGlobals.h:
	extern long 	gLongitude;			/ * centerpoint of map * /
	extern long 	gLatitude;
	extern unsigned long	gPixDegLatInt, gPixDegLatFract;
	extern unsigned long	gPixDegLonInt, gPixDegLonFract;
	extern unsigned long	gXPixel, gYPixel;
  */
  ...
  if (TIFFGetField (in, TIFFTAG_IMAGEDESCRIPTION, & imageDescription) == 1) {
    ...
    if (str && sscanf (imageDescription, "CART_RESOURCE(%d,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu)",
		       ...
		       & gPixDegLatInt, & gPixDegLatFract,  
		       & gXPixel, & gYPixel) == 9) {
      /*        printf ("exporting CART_RESOURCE tag: %s\n", imageDescription); */
      ...
      /*        printf ("Image Center lon %f lat %f\n", gLongitude / 3600000.0 - 180.0, gLatitude / 3600000.0 - 90.0); */
      /*        printf ("Image Center xpix %u ypix %u\n", gXPixel, gYPixel); */
      /*        printf ("pix/Deg Lon %f pix/Deg Lat %f\n", pixDegLon, pixDegLat); */

      /* make a tiepoint at (gXPixel, gYPixel, 0, gLongitude / 3600000.0 - 180.0, gLatitude / 3600000.0 - 90.0, 0) */
      {
	int part;
	double tiePoint [6];
	int size = 3;
...
#if 0
#define GT_CNT 8
	char * geotiffMetaNames [GT_CNT] = {"NITF_CORNERLAT1",
					    "NITF_CORNERLON1",
					    "NITF_CORNERLAT2",
					    "NITF_CORNERLON2",
					    "NITF_CORNERLAT3",
	...
	while (str && str < end) {
	  ...
	  if (p && p < end) {
		...
		if (! strcmp (nameBuf, coregMetaNames [i])) { /* if this is coreg meta data */
	      ...
	      /*	      for (i = 0; i < GT_CNT; i ++) / * for each value name */
	      if (strstr (nameBuf, "NITF_CORNER") || strstr (nameBuf, "CHIP_")) { /* if this is geotiff meta data */
		foundMetaName = 1;

		/* insert name/value into each image */
		for (outIndex = 0; outIndex < outCount; outIndex ++) {
		  if (isdigit (value [0]) && ! strstr (nameBuf, "CHIP_")) { /* int or real, but don't convert chips */
		    if (strchr (value, '.')) { /* real */
		      double doubleVal;
		      sscanf (value, "%lf", & doubleVal);
		      if (zladd (outunit [outIndex], "PROPERTY",
...
/*    printf ("there are %d input bands and %d out names\n", nbs, *numout); */
...
/* Multivalued label item structure */
	...
	if (!strcmp ("GEOTIFF", prop_names[subset]) || !strcmp ("COREG_META_DATA", prop_names[subset])) {
	  ...
	  while (TRUE)
		...
		if (strstr (key, "RPC_FIELD") || strstr (key, "NITF_") || strstr (key, "AMT_") || strstr (key, "CHIP_")) {
		  ...
		  if (strcasecmp (key, "ModelTransformationTag") && /* older code already handles these */
...
#if 0
		      This allows random values with special chars to confuse the resource parser.
			...
			if (value [0] == '(')
			  ...
			  int shortValue;
			  /*   			printf ("found GEOTIFF Short value with gk_key %d: %s=%s\n", gk_key, key, value); */
			  {
			    char * p = strstr (value, "(");
			    if (p)
			      * p = '\0';
			  ...
			  /*   			printf ("GTIFKeySet(gtif, %d, TYPE_SHORT, 1, %d);\n", gk_key, shortValue); */
			...
			* (strstr (value, ";gk_type=Double")) = '\0';
			if (value [0] == '(')
			  ...
			  double doubleValue;
			  /*   			printf ("found GEOTIFF Double value with gk_key %d: %s=%s\n", gk_key, key, value); */
			  {
			    char * p = strstr (value, "(");
			    if (p)
			      * p = '\0';
			...
			*p = '\0';
			/*   		      printf ("found GEOTIFF String value with gk_key %d: %s=%s\n", gk_key, key, value); */
...
/*  		printf ("zlinfo got key=%s, format=%s, maxlength=%d, nelement=%d\n", key, format, maxlength, nelement); */
...
void VicAddGeoTIFF(TIFF *out, GTIF *gtif)
{
...
#if 0
  /* several ways for datum spec. */
        
  p = ms_find(gtlabelstr,"WGS84");
  q = ms_find(gtlabelstr,"GEOGELLIPSOIDGEOKEY=7030");
  if (p!=0||q!=0)
    {	
      gcs = GCS_WGS_84;
      gcs_name = "WGS-84 Datum";
    }
  else if (ms_find(gtlabelstr,"WGS72")!=0)
    {	
      gcs = GCS_WGS_72;
      gcs_name = "WGS-72 Datum";
    }
  else if (ms_find(gtlabelstr,"NAD27")!=0)
    {	
      gcs = GCS_NAD27;
      gcs_name = "NAD 27 Datum";
    }
  else if (ms_find(gtlabelstr,"NAD83")!=0)
    {	
      gcs = GCS_NAD83;
  ...
  /* do the point/area type here, note VTIFF2 assumes geographic always area and projective
     always point, note that DTED is a counterexample to that assumption */
	
  p = ms_find(gtlabelstr,"GTRASTERTYPEGEOKEY=2");
  if (p!=0)
  ...
  p = ms_find(gtlabelstr,"GTMODELTYPEGEOKEY=2");
  if (p!=0)
    {
  ...
  else
    {
      ...
      p = ms_find(gtlabelstr,"PROJSTDPARALLEL1GEOKEY=");
      if (p!=0)
	{
	  parmval = ms_dnum(&p);
	  GTIFKeySet(gtif, ProjStdParallel1GeoKey, TYPE_DOUBLE, 1,parmval );
	}
      p = ms_find(gtlabelstr,"PROJSTDPARALLEL2GEOKEY=");
      if (p!=0)
	{
	  parmval = ms_dnum(&p);
	  GTIFKeySet(gtif, ProjStdParallel2GeoKey, TYPE_DOUBLE, 1,parmval );
	}
	   
      p = ms_find(gtlabelstr,"PROJCENTERLATGEOKEY=");
      if (p!=0)
	{
	  parmval = ms_dnum(&p);
	  GTIFKeySet(gtif, ProjNatOriginLongGeoKey, TYPE_DOUBLE, 1,parmval );
	}
	   
	  
      /* this was in the old program, but VICAR label doesnt have yet
	 ...
	 GTIFKeySet(gtif, ProjFalseOriginLatGeoKey, TYPE_DOUBLE, 1,parmvalues[0] );*/
      ...
      p = ms_find(gtlabelstr,"PROJFALSEEASTINGGEOKEY=");
      if (p!=0)
	{
	  parmval = ms_dnum(&p);
	  GTIFKeySet(gtif, ProjFalseOriginEastingGeoKey, TYPE_DOUBLE, 1,parmval );
	}
      p = ms_find(gtlabelstr,"PROJFALSENORTHINGGEOKEY=");
      if (p!=0)
	{
	  parmval = ms_dnum(&p);
	  GTIFKeySet(gtif, ProjFalseOriginNorthingGeoKey, TYPE_DOUBLE, 1,parmval );
	}
    }
#endif
}
...
#ifdef EXTENDED_TIFF
  /* we have to set this thing first */
  if (compression == COMPRESSION_JPL)
    {
      if (zvptst("NGT"))
	{
	  TIFFSetField(out, TIFFTAG_JPL_COMPRESSION, JPL_COMPRESSION_NGT);
	  ngtflag=1;
	}
      else
	TIFFSetField(out, TIFFTAG_JPL_COMPRESSION, JPL_COMPRESSION_JPL);
    }
#endif
...
int GetNULLS(unsigned char **nulltiles)
{
  ...
  /*nulls = (int *)malloc(sizeof(char)*numnulls);alz*/
...
int WriteTIFFData(int * inunit, TIFF *out, int nl, int ns, int nb)
{
  ...
  zvget(inunit[0],"PIX_SIZE",&pixsize, NULL);
  /* chunky = zvptst( "chunky" ) && (nb>1) && (pixsize==1); */
...
#ifdef EXTENDED_TIFF
  if (ngtflag)
    NGTSetEncoding(out, NGT_CODEC_NULL);
  else
    JPLSetEncoding(out, JPL_CODEC_NULL);
#endif 
}
