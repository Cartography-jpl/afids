...
void main44(void) {
  ...
  zvmessage((char*) "ibisclst2 version 2019-10-08 - rjb - 64-bit", (char*) " ");
  
  if ((zvparmd((char*) "distance", &dist, &cnt, &def, 1, 0)) != 1)
    zmabend((char*) "error parsing dist");

  if ((zvparmd((char*) "xcol", &xCol, &cnt, &def, 1, 0)) != 1)
    zmabend((char*) "error parsing xcol");

  if ((zvparmd((char*) "ycol", &yCol, &cnt, &def, 1, 0)) != 1)
    zmabend((char*) "error parsing ycol");

  if ((zvparmd((char*) "clustcol", &clustCol, &cnt, &def, 1, 0)) != 1)
    zmabend((char*) "error parsing clstcol");

  if (dist <= 0.0)
    zmabend((char*) "dist must be > 0.0");
  ...
  if ((status = IBISColumnGet(ibis, (char*) "FORMAT", fmtstring, xCol)) != 1)
    zmabend((char*) "error getting xcol column format");
  ...
  if ((status = IBISColumnGet(ibis, (char*) "FORMAT", fmtstring, yCol)) != 1)
    zmabend((char*) "error getting yCol column format");
