...
void main44(void)
{
   ...
   zifmessage("ibis2rpc version Fri Jan 11 2008");
   ...
   if (scale[0]<0.5)
     sprintf(buf,"NITF_CETAG=RPC00A");
   else
     sprintf(buf,"NITF_CETAG=RPC00B");
   ...
   for (i=0;i<13;i++)
      ...
      strcat(labelstr,"\n");
      if (i==0)
         sprintf(buf,"RPC_FIELD%d=%1.0f",i+1,scale[i+2]);
      else
         sprintf(buf,"RPC_FIELD%d=%7.6f",i+1,scale[i+2]);
      strcat(labelstr,buf);
      }
   ...
   for (i=0;i<80;i++)
      ...
      strcat(labelstr,"\n");
      if (coeff[i]>=0)
         sprintf(buf,"RPC_FIELD%d%d=+%16.14f",nlab,nix,coeff[i]);
      else
         sprintf(buf,"RPC_FIELD%d%d=%17.14f",nlab,nix,coeff[i]);
      strcat(labelstr,buf);
      }
