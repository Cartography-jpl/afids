...
//LinkedList_removeNode(LINKEDLIST *samples,struct node *t);
...
void printSample(SAMPLE *sample)
{
   ...
   printf("dn: ");
   for(i = 0; i < (int)sample->dns->size; i++) printf("%lf ", gsl_vector_get(sample->dns, (size_t)i));
   printf("\n");
}
...
VICAR_IMAGE** getVicarImages(int cnt)
{
   ...
   inps = (VICAR_IMAGE**)malloc(sizeof(VICAR_IMAGE*)*(long unsigned int)cnt);
   for(i = 0; i < cnt; i++)
...
void readSamples(LINKEDLIST *samples, VICAR_IMAGE **inps, int nDim, int nSamples)
{
   ...
   for(j = 0; j < nDim; j++)
   {
      ...
      gsl_vector_set(s->dns, (size_t)j, inps[j]->buffer[s->sample]);
   }
...
void removeZeros(LINKEDLIST *samples)
{
   ...
   while(n != NULL)
   {
      ...
      {
         if(gsl_vector_get(s->dns, (size_t)i) < 10E-10)
         {
            struct node *t;
...
//   SAMPLE **readSampleIndices;
   ...
   for(i = 0; i < nGridX; i++)
   {
      ...
      for(j = 0; j < nGridY; j++)
      {
         ...
         s->line = y;
         s->sample = x;
         s->dns = gsl_vector_alloc((size_t)nDim);
...
void outIBIS(LINKEDLIST *samples, int nDim, int nr)
{
   ...
   //   printf("outIBIS1 %d\n", samples->size);
   array = (struct node**)malloc(sizeof(struct node*)*(long unsigned int)samples->size);
   //   printf("outIBIS2\n");
   ...
   for(i = 0; i < nr; i++)
   {
      ...
      //      printf("outIBIS10\n");
      for(j = 2; j < nc; j++)
         IBISHELPER_setDouble(out, j, i, (double)gsl_vector_get(sample->dns, (size_t)j-2));
      //      printf("outIBIS11\n");
   }
   //   printf("outIBIS12\n");
...
{
   long seed;
   int status, inpCnt, dumdef, dumcnt;
   int nGridX, nGridY;
   char sampleType[20];
   char lowerType[20];
   VICAR_IMAGE **inps;
//   IBISStruct *out;
   ...
   //   struct node **nodeArray;
    ...
    zvmessage ("sampler - 10-17-2019 - rjb - 64-bit"," ");
    samples = 0;
