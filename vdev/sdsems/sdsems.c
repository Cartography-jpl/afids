...
void main44(void)
{
  ...
  zifmessage("SDSEMS version 2018-06-15");
  ...
  if (debLine) {
    sprintf(buf, "using WX %lf, WY %lf, DEBLINE %d, DEBSAMP %d", wX, wY, debLine, debSamp);
    zifmessage(buf);
  }
...
#define I(LINE,SAMP) (INPIX((LINE+1),(SAMP+1)))
  ...
  if (debLine) {
    ...
    for (line=debLine - 2; line < debLine + 1; ++line) {
      sprintf(buf, "LINE %6d: ", line + 1);
      for (samp=debSamp - 2; samp < debSamp + 1; ++samp) {
	sprintf(buf0, "%12f", INPIX(line, samp));
	strcat(buf, buf0);
      }
      zifmessage(buf);
    }
  ...
  for (line = 1; line < nl - 1; ++line)
	...
	for (winY = 0; winY < 3; ++winY) {
	  ...
	  sum += dif * dif;
	  if (debLine && line == debLine - 1 && samp == debSamp - 1) {
	    sprintf(buf, "LINE %d SAMP %d hi %f hm %f dif %f dif^2 %f (wX * winX * cosArad + wY * winY * sinArad) %f",
