...
void main44(void)
{
   ...
   int window,nline1,nsamp1,nline2,nsamp2,thresh,labnl,labns;
   int i_unit1,i_unit2,o_unit,*bptr,ll,lu,sl,su,llint,slint,icase;
   int ncomp,bigix,icur,iprev,pcomp,iline,jsamp,val1,lval,uval;
   int itemp,hptr,icomp,qcomp,notfound,ptrl,sptr,doedge;
   int ecount,becount,mcount,bmcount,d1,d2,dopow2,ocptr1,ocptr2;
   short int **inbuf,**inbuf2,*inbufr,*ptr,val,*hval2,*boost,*ocnl,*ocns;
   int *hline,*hsamp,*hcomp,*inbufcomp,*bsum1,*bsum2,*bcount,*hval;
   int *shline,*shsamp,*shcomp,*shresid,curline2,ocnthrsh,noneg;
   char fmt_str[10];
   float demfac;
   ...
   zifmessage("demfix version Thu Jan 10 2008");
   ...
   if (doedge==(-1)||zvptst("edgeauto"))
      ...
      if (((float)becount/(float)ecount)>(4.0*(float)(bmcount+2)/(float)mcount))
         {
         doedge = 1;
         printf("performing edge fix, statistics edge=%d/%d interior=%d/%d\n",
            becount,ecount,bmcount,mcount);
         }
      else
         {
         doedge = 0;
         printf("no edge fix, statistics edge=%d/%d interior=%d/%d\n",
            becount,ecount,bmcount,mcount);
         }
   ...
   for (iline=0;iline<nline1;iline++)
      {
      if (iline%100==0)
         printf("line=%d pixels=%d component=%d\n",iline,bigix,ncomp);
      status = zvread(i_unit1,inbuf[icur],"LINE",iline+1, NULL);
      if (doedge)
         {
         inbuf[icur][0] = thresh;
...
if (inpcount>1)
   {
   printf("reading secondary dem file\n");
   /*for (i=0;i<bigix;i++) 
   ...
   printf("final update of primary file\n");
   if (inpcount>1) zvclose(i_unit2, NULL);
   status = zvunit( &i_unit1, "INP", 1, NULL);
   status = zvopen( i_unit1, "OPEN_ACT", "SA", "IO_ACT", "SA",
			"U_FORMAT","HALF", NULL);
