...
/* program zrelax2                                                      */
/************************************************************************/
/*  03-08 ...alz... initial version of zrelax.c         */
/*  04-01 ...alz... initial version of zrelax2.c       */
/*  Fri Dec 28 2007 wlb switched to USES_ANSI_C AND LIB_CARTO; misc cleanup */
/************************************************************************/

void main44(void)
{
   int i,j,unit,ibis,status,clen,cols[9],colcount,coldef,val;
   int niter,nsum,ibig,pval,pvaltop;
   int *indx,*nbrindx,*reversix;
   double *zframe,*znbr,*deltaz,*wtavgz,*curwtavgz,*curzframe,*curznbr;
   double *csum,*corr,sum,val0,relaxconst,lastsum;
    
   zifmessage("zrelax2 version Fri Dec 28 2007");
   
   /* get the basic parameters */
   
   zvparm("cols",cols,&colcount,&coldef,9,0);
   relaxconst = 2.0;
   ...
   mz_alloc1((unsigned char **)&nbrindx,clen,4);
   mz_alloc1((unsigned char **)&reversix,clen,4);
   ...
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnRead(ibis,(char*) nbrindx,cols[2],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
   
   status = IBISColumnSet(ibis,"U_FORMAT","DOUB",cols[3]);
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnRead(ibis,(char*) znbr,cols[3],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
   
   status = IBISColumnSet(ibis,"U_FORMAT","DOUB",cols[4]);
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnRead(ibis,(char*) deltaz,cols[4],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
   
   status = IBISColumnSet(ibis,"U_FORMAT","DOUB",cols[5]);
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnRead(ibis,(char*) wtavgz,cols[5],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
   ...
   for (i=0;i<clen;i++)
      for (j=0;j<clen;j++)
         if (indx[i]==nbrindx[j]&&indx[j]==nbrindx[i]) reversix[i] = j;
   ...
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnWrite(ibis,(char*) curwtavgz,cols[6],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
     
   status = IBISColumnSet(ibis,"U_FORMAT","DOUB",cols[7]);
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnWrite(ibis,(char*) curzframe,cols[7],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
   
   status = IBISColumnSet(ibis,"U_FORMAT","DOUB",cols[8]);
   if (status!=1) IBISSignal(ibis,status,1);
   status = IBISColumnWrite(ibis,(char*) curznbr,cols[8],1,clen);
   if (status!=1) IBISSignal(ibis,status,1);
